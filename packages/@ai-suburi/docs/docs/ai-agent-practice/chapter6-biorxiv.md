...
##### FT 対応モデルとコスト比較（2026 年 4 月時点）

OpenAI の全モデルがファインチューニングに対応しているわけではありません。2026 年 4 月時点では、**gpt-4 世代のモデルのみ** が SFT（Supervised Fine-Tuning）に対応しています。gpt-5 系や gpt-5.4 系は推論用途のみで、FT は未提供です。

**SFT 対応モデル:**

| モデル | トレーニング ($/1M tok) | FT 後推論 入力 ($/1M tok) | FT 後推論 出力 ($/1M tok) | 特徴 |
| --- | --- | --- | --- | --- |
| **gpt-4.1-nano** | **$0.20** | $0.15 | $0.60 | 最安 FT、1M コンテキスト |
| gpt-4.1-mini | $0.80 | $0.60 | $2.40 | バランス型 |
| gpt-4.1 | $3.00 | $3.00 | $12.00 | 高品質 |
| gpt-4o-mini | $3.00 | $0.30 | $1.20 | 実績豊富 |
| gpt-4o | $25.00 | $3.75 | $15.00 | 高品質だがコスト高 |

**FT 非対応モデル（推論専用）:**

| モデル | 理由 |
| --- | --- |
| gpt-5 / gpt-5-mini / gpt-5-nano | FT 未提供 |
| gpt-5.4 / gpt-5.4-mini / gpt-5.4-nano | FT 未提供（最新モデルのため） |

ただし、FT **非対応** のモデルでも **学習データの生成** には使えます。つまり「gpt-5-nano で高品質な学習データを安く作り、gpt-4.1-nano を FT する」という組み合わせが可能です。

**おすすめの組み合わせ（学習データ 9 万件 = 1.8 万論文 × 5 クエリの場合）:**

| 組み合わせ | データ生成コスト | トレーニングコスト | 合計 | おすすめ |
| --- | --- | --- | --- | --- |
| gpt-5-nano → **gpt-4.1-nano** FT | ~$3.5 (500円) | ~$9 (1,300円) | **~$12.5 (1,800円)** | 💰 最安 |
| gpt-5-mini → **gpt-4.1-nano** FT | ~$27 (3,900円) | ~$9 (1,300円) | **~$36 (5,200円)** | ⭐ バランス |
| gpt-5-mini → **gpt-4.1-mini** FT | ~$27 (3,900円) | ~$36 (5,200円) | **~$63 (9,200円)** | 🏆 高品質 |
| gpt-5-nano → **gpt-4o-mini** FT | ~$3.5 (500円) | ~$135 (2万円) | **~$139 (2万円)** | 🔬 安定志向 |

FT 済みモデルの推論コスト増はごくわずかです。たとえば gpt-4.1-nano の FT 済みモデルは、エージェント 1 回の実行あたり約 +$0.0008（約 0.1 円）の追加コストで済みます。
...
```bash
# .env でモデル名を差し替えるだけ
OPENAI_FAST_MODEL=ft:gpt-4.1-nano-2025-04-14:your-org::xxxxxxxx
```
