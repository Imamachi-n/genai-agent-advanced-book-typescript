...
なお、OpenAI の Fine-tuning API は **OpenAI のクラウドサーバー上で学習を実行する**ため、ローカル環境に GPU や高スペックなマシンは不要です。普段の開発に使っているマシンで十分です。GPU が必要になるのは、オープンソースモデル（Llama 等）を LoRA / QLoRA で自前のサーバーで学習させる場合に限られます。
