...
なぜ三層構造にするのでしょうか？ ユーザーの調査目的は「LLM エージェントの評価方法」のように抽象的であることが多く、これを具体的なサブタスクに分解し、各タスクに対して複数の論文を検索・分析する必要があります。さらに、各論文の分析ではセクション選択 → 十分性チェック → 要約という反復的な処理が発生します。

これらの処理を階層的なエージェントで分離することで、以下のメリットが得られます。

- **責務の明確化**: 各エージェントが担当する範囲が限定され、プロンプトやロジックがシンプルになる
- **並列実行による高速化**: 複数の論文を同時に分析できる（Send API の活用）
- **再利用性**: PaperAnalyzerAgent は単体でも利用可能
...
マルチエージェントシステムでは、ヒアリング結果・検索クエリ・論文データ・評価結果など、多種多様なデータがエージェント間を流れます。これらのデータ構造を事前に厳密に定義しておかないと、あるエージェントの出力が別のエージェントの期待する形式と合わず、実行時エラーやデータ欠損が発生します。

Chapter 5 と同様に **Zod スキーマ** を使って型定義を行い、Structured Outputs で LLM の出力を型安全に構造化します。また、論文データを LLM に渡す際に **XML 形式** に変換するヘルパー関数も用意します。XML はタグで構造を表現できるため、LLM がデータの各フィールドを正確に認識しやすくなります。
...
テーブルに加えて、以下の 2 つのヘルパー関数も定義します。

- **`arxivPaperToXml()`**: 論文データを XML 形式に変換する関数。LLM のプロンプトに論文情報を構造化して渡す際に使用します
- **`formatTimeRange()`**: 日付範囲を arXiv API のクエリ形式（`YYYYMMDD+TO+YYYYMMDD`）に変換する関数
...
なぜ複数の LLM を使い分けるのでしょうか？ すべての処理に高性能モデルを使うとコストが膨大になります。一方、軽量モデルだけでは複雑な推論やレポート生成の品質が不足します。そこで、タスクの特性に応じて **3 種類の LLM インスタンス** を使い分けます。
...
ユーザーの調査目的は「LLM エージェントの評価方法」のように自然言語で表現されますが、arXiv API はキーワードベースの検索エンジンです。自然言語をそのまま検索クエリにしても、関連性の高い論文が上位に来るとは限りません。

そこで、**LLM を活用した 4 段階の最適化パイプライン** で検索精度を高めます。
...
```mermaid
flowchart LR
    subgraph llm_opt["LLM によるクエリ最適化"]
        direction TB
        A["フィールド選択<br/>（arXiv カテゴリ判定）"] --> B["日付範囲選択<br/>（時間範囲推定）"]
        B --> C["クエリ展開<br/>（検索クエリ生成）"]
    end

    subgraph search["検索 & リランキング"]
        direction TB
        D["arXiv API 検索"] --> E["Cohere Rerank<br/>（関連度スコア順）"]
        E --> F["スコア 0.7 以上<br/>の論文のみ返却"]
    end
...
:::tip Cohere Rerank のスコアしきい値

`RELEVANCE_SCORE_THRESHOLD = 0.7` は、関連度が低い論文をフィルタリングするためのしきい値です。この値を下げると検索結果が増えますが、ノイズも増えます。逆に上げすぎると関連性の高い論文も除外されてしまうため、0.6〜0.8 の範囲で調整するのが実用的です。
...
前セクションで arXiv API から論文メタデータ（タイトル・アブストラクト・PDF リンク）を取得できるようになりました。しかし、論文の本文は PDF 形式のため、LLM が直接テキストとして読み取ることができません。

そこで **Jina Reader API**（`https://r.jina.ai`）を使って PDF URL から直接 Markdown テキストを取得します。一度変換した論文はローカルにキャッシュし、同じ論文を複数回変換する無駄を防ぎます。

:::caution Jina Reader API の利用上の注意

Jina Reader API は無料プランでも利用できますが、レート制限があります。大量の論文を一度に変換する場合はエラーが発生する可能性があるため、`maxPapers` 設定で同時変換数を制御しています（デフォルト: 3）。
...
前セクションで PDF を Markdown に変換できるようになりましたが、学術論文は数十ページにも及ぶことがあり、全文をそのまま LLM に渡すとコンテキストウィンドウを圧迫します。また、論文のすべてのセクションが調査タスクに関連するとは限りません。

そこで、Markdown をセクション単位に分割し、LLM が**タスクに関連するセクションだけを選択的に読み込む**仕組みを実装します。この仕組みは後述の PaperAnalyzerAgent（6-7）のセクション選択・十分性チェックの基盤となります。
...
ここまでのセクション（6-1〜6-5）で、型定義・検索・PDF 変換・パースといった**基盤となる部品**が揃いました。ここからは、これらの部品を組み合わせて**エージェントのワークフロー**を構築していきます。

まず本セクションでは、ユーザーの入力を検索可能なサブタスクに変換するまでの 3 つのチェーンを実装します。これらは ResearchAgent（6-11）のワークフローにおけるフロントエンド部分に相当し、**曖昧なユーザーの要求を、arXiv で検索できる具体的なタスクに落とし込む**役割を担います。

プロンプトテンプレートの読み込みと XML 変換のヘルパー関数も合わせて紹介します。
...
前セクションで検索クエリの生成まで完成しました。ここからは、実際に論文を読んで分析するエージェントを実装します。

PaperAnalyzerAgent は**個別の論文に対する分析を担当するサブエージェント**です。6-5 で実装した `MarkdownParser` を活用し、以下の 3 つのチェーンを組み合わせた LangGraph サブグラフとして実装されています。
...
PaperAnalyzerAgent は個別の論文を分析しますが、「どの論文を分析対象にするか」を決定するのは `PaperSearchAgent` の役割です。PaperSearchAgent は、6-6 で分解されたサブタスクごとに論文検索 → PDF 変換 → 分析の一連の処理を管理するサブエージェントです。

ポイントは **LangGraph の `Send` API** を使った動的な並列処理です。通常の `addEdge` ではノード間の接続が静的に決まりますが、`Send` を使うと**実行時に動的に複数のノードインスタンスを起動**できます。`PaperProcessor` が検索・PDF 変換を行った後、各論文に対して `Send` で `analyze_paper` ノードを並列起動します。これにより、複数の論文を同時に分析できます。
...
:::info Send API と reducer の関係

`processingReadingResults` の reducer は `(prev, next) => [...prev, ...next]` で**追加蓄積**型になっています。Send API で並列起動された各 `analyze_paper` ノードの結果が、この reducer によって 1 つの配列に順次マージされます。LangGraph の State 設計では、並列ノードの結果を集約する際にこの「追加蓄積型 reducer」パターンがよく使われます。
...
PaperSearchAgent による論文の検索・分析が完了しましたが、1 回の検索で十分な情報が得られるとは限りません。「サーベイ論文は見つかったが、具体的な実験結果が不足している」といったケースは頻繁に発生します。

`TaskEvaluator` は、論文検索・分析の結果がユーザーのゴールに対して十分かどうかを LLM が評価します。情報が不足している場合は `decompose_query` に戻り、不足分を補う新しいサブタスクで再検索を行います。この**再検索ループ**（最大 3 回）により、最終レポートの品質を段階的に向上させます。
...
タスク評価で十分な情報が集まったと判定されると、いよいよ最終レポートの生成フェーズに入ります。

`Reporter` は、すべての調査結果を集約して最終的なマークダウンレポートを生成します。レポート生成には **Claude Sonnet 4**（`ChatAnthropic`）を使用しています。Claude Sonnet 4 は `maxTokens: 8192` で長文出力に対応しており、複数の論文を横断的に分析・比較する学術レポートの生成に適しています。
...
[research-agent] |--> human_feedback
User Feedback: 機械学習の分野で、特にベンチマークデータセットに焦点を当てて調べてください
[research-agent] |--> user_hearing
[research-agent] |--> goal_setting
[research-agent] |--> decompose_query
[research-agent] |--> paper_search_agent
[paper-search-agent] |--> search_papers
[arxiv-searcher] Searching for papers: https://export.arxiv.org/api/query?...
[arxiv-searcher] Found 10 papers.
[paper-search-agent] |--> analyze_paper
[paper-analyzer-agent] |--> set_section
[paper-analyzer-agent] |--> check_sufficiency
[paper-analyzer-agent] |--> summarize
[paper-search-agent] |--> organize_results
[research-agent] |--> evaluate_task
[research-agent] |--> generate_report

# LLMエージェントの評価方法に関する調査レポート

## 1. はじめに
LLMエージェントの評価は、近年の人工知能研究において...
（以下、マークダウン形式の詳細なレポートが出力されます）
```

:::tip human-in-the-loop の動作

`interrupt()` で中断されたワークフローは、`Command({ resume: userInput })` で再開されます。`MemorySaver` チェックポインターにより中断時点の状態が保存されるため、再開後は中断直前の状態から処理を継続できます。
...
- NaturalIntelligence. [fast-xml-parser](https://github.com/NaturalIntelligence/fast-xml-parser) - arXiv API の XML レスポンスパースに使用（6-3）
